...
public class DragLayout extends FrameLayout {
    ...
    private TouchRelativeLayout vg_main;
    ...
    protected void onFinishInflate() {
        ...
        if (isShowShadow) {
            ...
            iv_shadow.setImageResource(R.drawablea.shadow);
            LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            addView(iv_shadow, 1, lp);
        }
        vg_left = (ViewGroup) getChildAt(0);
        vg_main = (TouchRelativeLayout) getChildAt(isShowShadow ? 2 : 1);
    ...
    public boolean canTouchEvent(MotionEvent ev) {
        boolean can = false;
        if (getStatus() == Status.Open) {
            ...
            int actionBarHeight = getContext().getResources().getDimensionPixelSize(R.dimen.actionbar_height);

            if (touchX <= actionBarHeight) {
                can = true;
            } else if (touchY <= actionBarHeight) {
                can = true;
            }
        }
        return can;
    ...
    private void animateView(float percent) {
        ...
        LogUtil.d(f1 + "");
        vg_left.setTranslationX(-vg_left.getWidth() / 2.3f + vg_left.getWidth() / 2.3f * percent);
